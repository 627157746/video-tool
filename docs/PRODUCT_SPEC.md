...
> 状态：**v0.3 已交付**（应用版本 `0.3.4`）；v0.1 为基线，第 14 节为 v0.2 已实现能力说明（原路线图，非未交付清单）  
> 目标用户：仅自己使用，架构按「可演进到小范围分发」预留  
> 版本：`0.3.4` = `0.3.3` + 更新 helper 启动路径修复（决策日志 29 条）；后置能力仍见各节「非目标」  
...
27. **下载误报 / 更新重启加固 / 二次确认**（2026-07-27，应用 **`0.3.2`**）：下载进度写盘节流与 `load_job` 短重试，避免并发 `source.json` 读写误报「任务不存在」；更新 helper 改为独立 `.cmd` 等待 PID 后再安装并拉起；删除/清理媒体/重跑媒体与转写总结/停止录制/导出/导入配置/工作区修复/重建索引等高影响操作统一二次确认；忽略 `.cursor/mcp.json`  
28. **应用内确认 UI 与覆盖面**（2026-07-28，应用 **`0.3.3`**）：`confirmAction` 走任务中心风格模态（非系统对话框）；StrictMode 下队列不丢首次确认；设置草稿删除 Provider/模板/分组与校对「放弃修改」需确认；重跑/重试与已校对风险合并为单次确认  
29. **更新 helper 启动修复**（2026-07-28，应用 **`0.3.4`**）：禁止 `cmd /C start "video-tool-update" …`（参数转义会把标题当成可执行文件，报找不到 `\video-tool-update\`）；改为 `cmd /C <helper.cmd>` + 分离进程标志，批处理写 CRLF
