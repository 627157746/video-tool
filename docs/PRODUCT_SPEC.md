...
> 状态：v0.1 能力共识与实现基线已锁定；**v0.2+ 路线图**已写入第 14 节（来源：产品 grill + 后续功能选型）  
> 目标用户：仅自己使用，架构按「可演进到小范围分发」预留  
> 版本目标：v0.1 = **接近自用定稿**（非「仅骨架能跑」）；v0.2+ 见第 14 节分期  
...
### 2.4 v0.2+ 仍明确不做（路线图边界）

以下即使进入 v0.2+ 讨论，也**不作为承诺**，避免范围膨胀：
...
- v0.2+ 能力以第 14 节为范围权威；开 task / 改代码前核对 MVP 与非目标，避免把后置项当本期交付。
...
**后续实现与拆分请以第 14 节为准**，不再把未列能力默认为 v0.1 缺口。
...
- 第 14 节能力**默认未交付**；完成某一项时：更新实现勾选（可在本节追加 checkbox 列表或独立 milestone 文档）、补目录契约、补测试与脱敏审查。  
- 自动化测试仍不替代 sidecar / 网络 / API Key / Cookie 有效性等环境验收。  
- 若调整分期或砍掉能力：**先改本文第 14 节与第 11 节决策日志，再改代码**。
